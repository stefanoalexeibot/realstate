    ...
    <div className="p-6 sm:p-8">
      <div className="mb-8">
        <div className="flex items-center gap-2.5 mb-1">
          ...
          <h1 className="font-heading font-bold text-2xl text-cima-text">Pipeline de ventas</h1>
        </div>
        <p className="text-sm text-cima-text-muted">
          {leads.length} propietarios en proceso · Arrastra las tarjetas o usa los botones para avanzar etapas.
        </p>
      </div>
        ...
        {STAGES.map((stage, stageIdx) => {
          ...
          return (
            <div key={stage.key} className="flex-shrink-0 w-60">
              {/* Column header */}
              <div className={`rounded-t-xl border ${stage.color} px-3 py-2.5 flex items-center justify-between`}>
                <div className="flex items-center gap-2">
                  <div className={`h-2 w-2 rounded-full ${stage.dot}`} />
                  <span className="font-mono text-[10px] tracking-[0.15em] text-cima-text uppercase">{stage.label}</span>
                </div>
                  ...
                  cards.map((lead) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className={`inline-flex px-1.5 py-0.5 rounded text-[9px] font-mono border ${
                          lead.operation_type === "venta"
