      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-xl border border-cima-border bg-cima-card p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-cima-text-muted mb-1.5">Área m²</label>
        ...
        <div className="rounded-xl border border-cima-border bg-cima-card p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {STATUS_OPTS.map((s) => (
              ...
              >
                <p className="text-xs font-semibold">{s.key === "active" ? "Activa" : s.key === "inactive" ? "Inactiva" : s.key === "sold" ? "Vendida" : "Rentada"}</p>
                <p className="text-[10px] text-cima-text-dim mt-0.5 leading-tight">
                  {s.key === "active" ? "Visible en catálogo" : s.key === "inactive" ? "Oculta al público" : s.key === "sold" ? "Operación cerrada" : "Renta concluida"}
                </p>
              </button>
            ))}
          </div>
        ...
        <div className="flex items-center gap-3 pb-8">
          <button type="submit" disabled={loading}
            ...
            {loading ? <><Loader2 className="h-4 w-4 animate-spin" />{progress ?? "Guardando…"}</> : <><Building2 className="h-4 w-4" />Publicar propiedad</>}
          </button>
