...
export async function POST(req: Request) {
  try {
    ...
    // Use the verified 'gemini-flash-latest' alias
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        lastError = e;
        console.error(`AI Fill attempt ${i + 1} failed:`, e.message);
    ...
    if (!responseText) {
      return NextResponse.json({ error: "La IA no pudo responder tras varios intentos. " + lastError?.message }, { status: 503 });
    }
