...
export default function InteractiveEcosystem() {
    ...
    return (
        <div className="relative w-full aspect-square md:aspect-video max-w-[800px] mx-auto flex items-center justify-center py-10 md:py-20 overflow-visible">
            {/* Background Glows */}
            ...
            <div className="relative z-10">
                <motion.div
                    ...
                    className="w-24 h-24 md:w-32 md:h-32 rounded-full bg-cima-gold/10 border border-cima-gold/30 backdrop-blur-3xl flex items-center justify-center relative shadow-[0_0_50px_rgba(234,179,8,0.2)]"
                >
                    <div className="absolute inset-0 rounded-full animate-ping bg-cima-gold/20 opacity-20" style={{ animationDuration: '3s' }} />
                    <Zap className="h-8 w-8 md:h-10 md:w-10 text-cima-gold" />
                    <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap">
                        <span className="font-mono text-[9px] md:text-[10px] font-black text-cima-gold tracking-[0.4em] uppercase">Core AI</span>
                    </div>
                </motion.div>
            </div>
            ...
            {nodes.map((node, i) => (
                ...
                >
                    <div className="flex flex-col items-center gap-2 md:gap-4">
                        <div
                            className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-cima-card/50 border border-cima-border backdrop-blur-md flex items-center justify-center transition-all duration-500 md:group-hover:scale-110 group-hover:border-[currentColor] shadow-2xl overflow-hidden relative"
                            style={{ color: node.color }}
                        >
                            <div className="absolute inset-0 bg-[currentColor] opacity-0 group-hover:opacity-10 transition-opacity" />
                            {React.cloneElement(node.icon as React.ReactElement, { className: "h-5 w-5 md:h-6 md:w-6" })}
                        </div>
                        <div className="text-center max-w-[80px] md:max-w-[120px] transition-all duration-500 md:group-hover:translate-y-[-4px]">
                            <p className="text-[9px] md:text-[11px] font-black text-cima-text uppercase tracking-wider mb-1 leading-tight">{node.title}</p>
