        ...
        <div className="p-6 sm:p-8 max-w-6xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Link href="/admin/visitas">
                    <div className={`rounded-xl border ${pendingVisits > 0 ? "border-amber-500/20 bg-amber-500/5" : "border-cima-border bg-cima-card"} p-5 h-full hover:opacity-80 cursor-pointer transition-colors`}>
                        <div className="flex items-center justify-between mb-3">
                            <p className="text-xs text-cima-text-muted">Visitas pendientes</p>
                            <Calendar className={`h-4 w-4 shrink-0 ${pendingVisits > 0 ? "text-amber-400" : "text-cima-text-muted"}`} />
                        </div>
                        <p className={`font-heading font-bold text-3xl leading-none ${pendingVisits > 0 ? "text-amber-400" : "text-cima-text-muted"}`}>
